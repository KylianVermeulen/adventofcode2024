...
struct AdventOfCode: AsyncParsableCommand {
  ...
  func run() async throws {
    ...
    for challenge in challenges {
      ...
      if benchmark {
        let timing1Ms = Double(timing1.components.seconds) * 1000 + Double(timing1.components.attoseconds) / 1_000_000_000_000_000
        let timing2Ms = Double(timing2.components.seconds) * 1000 + Double(timing2.components.attoseconds) / 1_000_000_000_000_000
        
        print("Part 1 took \(String(format: "%.2f", timing1Ms))ms, part 2 took \(String(format: "%.2f", timing2Ms))ms.")
#if DEBUG
